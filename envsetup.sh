...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the VoltageOS-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/voltage/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/voltage/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
